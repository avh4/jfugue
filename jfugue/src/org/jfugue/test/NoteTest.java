...
public class NoteTest {
	...
	public void testIsAdjustedForKey() {
		Note note = new Note();
		assertFalse("Note should be not be a adjusted for key by default",
				note.isAdjustedForKey());
	}
	...
	public void testNoteCTorWithNoteArg() {
		Note n1 = new Note((byte) 50, 0.5, NOTE_DEFAULT_VELOCITY,
				NOTE_DEFAULT_VELOCITY);
		Note n2 = new Note(n1);
		assertEquals(
				"Note constructed with Note argument should match argument",
				n1, n2);
	}
	...
	public void testChords() {
		...
		assertArrayEquals("MAJ intervals should be 4 & 7 semitones",
				new byte[] { 4, 7 }, chords.get("MAJ"));
	}

	@Test
	public void testEqualsSameNote() {
		String n1 = "Cb4/0.5";
		assertEquals("Notes with the same data should be equal",
				Note.createNote(n1), Note.createNote(n1));
	}

	@Test
	public void testEqualsDifferentValue() {
		assertFalse("Notes with different values should not be equal", Note
				.createNote("Cb4/0.5").equals(Note.createNote("D")));
	}

	@Test
	public void testGetVerifyString_Simple() {
		assertEquals(
				"Note: value=60, duration=0.25, startTie=F, endTie=F, attack=64, decay=64, isFirst=T, isParallel=F, isSequential=F",
				new Note((byte) 60).getVerifyString());
	}
	...
	public void testGetVerifyString_startTie() {
		...
		assertEquals(
				"Note: value=60, duration=0.25, startTie=T, endTie=F, attack=64, decay=64, isFirst=T, isParallel=F, isSequential=F",
				note.getVerifyString());
	}
	...
	public void testGetVerifyString_endTie() {
		...
		assertEquals(
				"Note: value=60, duration=0.25, startTie=F, endTie=T, attack=64, decay=64, isFirst=T, isParallel=F, isSequential=F",
				note.getVerifyString());
	}
	...
	public void testGetVerifyString_Parallel() {
		...
		assertEquals(
				"Note: value=60, duration=0.25, startTie=F, endTie=F, attack=64, decay=64, isFirst=F, isParallel=T, isSequential=F",
				note.getVerifyString());
	}
	...
	public void testGetVerifyString_Sequential() {
		...
		assertEquals(
				"Note: value=60, duration=0.25, startTie=F, endTie=F, attack=64, decay=64, isFirst=F, isParallel=F, isSequential=T",
				note.getVerifyString());
	}
	...
	public void testGetVerifyStringFromFourArgConstructor() {
		assertEquals(
				"Note: value=51, duration=0.5, startTie=F, endTie=F, attack=53, decay=54, isFirst=T, isParallel=F, isSequential=F",
				new Note((byte) 51, 0.5, (byte) 53, (byte) 54)
						.getVerifyString());
	}
	...
	public void testAdjustForKey_Rest() {
		...
		note.setRest(true);
		assertEquals(note,
				note.adjustForKey(new KeySignature((byte) 1, (byte) 1)));
	}
	...
	public void testGetMusicString_NonDefault_Attack() {
		assertEquals("C5qa5", new Note((byte) 60, 32, (byte) 5,
				Note.DEFAULT_VELOCITY).getMusicString());
	}

	@Test
	public void testGetMusicString_NonDefault_Decay() {
		assertEquals("C5qd5", new Note((byte) 60, 32, Note.DEFAULT_VELOCITY,
				(byte) 5).getMusicString());
	}

	@Test
	public void testGetMusicString_NonDefault_AttackAndDecay() {
		assertEquals("C5qa6d7",
				new Note((byte) 60, 32, (byte) 6, (byte) 7).getMusicString());
	}
