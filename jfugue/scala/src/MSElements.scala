...
abstract class Element {
  def eval(env : Environment) : JFugueElement
}

case class Voice(voice : ByteExp) extends Element {
  def eval(env : Environment) = new org.jfugue.elements.Voice(voice.eval(env))
  def getVoiceExp() = voice
}
case class Tempo(tempo : IntExp) extends Element {
  def eval(env : Environment) = new org.jfugue.elements.Tempo(tempo.eval(env))
}
case class Instrument(instrument : ByteExp) extends Element {
  def eval(env : Environment) = new org.jfugue.elements.Instrument(instrument.eval(env))
}
case class Layer(layer : ByteExp) extends Element {
  def eval(env : Environment) = new org.jfugue.elements.Layer(layer.eval(env))
}
case class KeySignature(letterNote : String, scale : String) extends Element {
  def eval(env : Environment) = new org.jfugue.elements.KeySignature(letterNote, scale)
}
case class Controller(index : IntExp, value : IntExp) extends Element {
  def eval(env : Environment) = new org.jfugue.elements.Controller(index.eval(env), value.eval(env))
}
case class Time(time : LongExp) extends Element {
  def eval(env : Environment) = new org.jfugue.elements.Time(time.eval(env))
}
case class SystemExclusive(radix: Int, bytes: List[Byte]) extends Element {
  def eval(env : Environment) = new org.jfugue.elements.SystemExclusive(bytes.toArray)
}
case class PolyphonicPressure(key : ByteExp, pressure : ByteExp) extends Element {
  def eval(env : Environment) = new org.jfugue.elements.PolyphonicPressure(key.eval(env), pressure.eval(env))
}
case class ChannelPressure(pressure : ByteExp) extends Element {
  def eval(env : Environment) = new org.jfugue.elements.ChannelPressure(pressure.eval(env))
}
case class PitchBend(lsb : ByteExp, msb : ByteExp) extends Element {
  def eval(env : Environment) = new org.jfugue.elements.PitchBend(lsb.eval(env), msb.eval(env))
}
case class Measure() extends Element {
  def eval(env : Environment) = new org.jfugue.elements.Measure()
}
case class Note(value : NoteValue, chord : Option[AbstChord], duration : Option[NoteDuration], velocity : Option[AbstVelocity]) extends Element {
  def eval(env : Environment) = {
    val (noKey: Boolean, nVal: Int) = value match {
      case LetterNote(l, v, o, n) => (n,
	    			      o match {
    ...
    val dur = duration match {
      case Some(dd) => dd.getDecimalDuration
      case None => JFugueDefinitions.DEFAULT_NOTE_DECIMAL_DURATION
    }
...
case class DictAdd(key : String, value : String) extends Element {
	def eval(env : Environment) = {
	  new org.jfugue.elements.DictAdd(key, value)
	}
}
//  case class AnyElem(s : Any) extends Element
case class Comment(comment : String) extends Element {
	def eval(env : Environment) = new org.jfugue.elements.Comment(comment)
}
